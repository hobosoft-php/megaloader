...
class JuggleCode extends PHPParser_PrettyPrinter_Zend
{
    ...
    public function setMasterfile($masterfile)
    {
        if (is_file($masterfile) && is_readable($masterfile)) {
            ...
            LogMore::err('Invalid file passed: %s', $masterfile);
        }
    }
    ...
    {
        return $this->saveMethodHandling(self::JC_OPPRESS, $instanceOrClass, $method, $expression);
    }
    ...
    {
        return $this->saveMethodHandling(self::JC_REPLACE, $instanceName, $method, $expression);
    }
     ...
     */
    private function saveMethodHandling($type, $instanceOrClass, $method, $expression = null)
    {
        ...
        $this->handleMethodCalls[$instanceOrClass][$method] = array('type' => $type, 'expression' => $expression);
        LogMore::debug('Method to handle (instance/method/type): %s/%s/%d', $instanceOrClass, $method, $type);
    }
    ...
    {
        return isset($this->handleMethodCalls[$instance][$method]['type']) ? $this->handleMethodCalls[$instance][$method]['type'] : 0;
    }
    ...
    {
        return isset($this->handleMethodCalls[$instance][$method]['expression']) ? $this->handleMethodCalls[$instance][$method]['expression'] : 0;
    }
    ...
    public function pExpr_FuncCall(PHPParser_Node_Expr_FuncCall $node)
    {
        ...
        if (in_array($functionName, $this->oppressedFunctionCalls)) {
            ...
            $code = $this->formatExpression($this->replacedFunctionCalls[$functionName], $node);
            $this->raiseCalledFunctionsCounter($this->replacedFunctionCalls[$functionName]);
        } else {
    ...
    private function formatExpression($expression, $node)
    {
        ...
        return str_replace($args_keywords, $args, $expression);
    }
    ...
    public function pExpr_MethodCall(PHPParser_Node_Expr_MethodCall $node)
    {
        ...
        LogMore::debug('Name of method and instance to call: %s, %s', $method, $instance);
    ...
    public function pExpr_StaticCall(PHPParser_Node_Expr_StaticCall $node)
    {
        ...
        $method = $node->name;
        LogMore::debug('Name of static method and class to call: %s, %s', $method, $class);
    ...
    private function printMethodCall($instanceOrClass, $method, $node)
    {
        ...
        if ($type == self::JC_OPPRESS) {
            ...
            $code = $this->formatExpression($this->getMethodHandlingExpression($instanceOrClass, $method), $node);
        }
        return $code;
    }
    ...
    {
        return array_diff($this->definedFunctions, array_keys($this->calledFunctions));
    }
}